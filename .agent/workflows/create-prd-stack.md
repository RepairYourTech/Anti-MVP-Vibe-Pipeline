...
This axis applies to all visual surfaces (web, desktop, mobile). For CLI-only
projects, the skill includes a CLI variant.

Read `.agent/skills/design-direction/SKILL.md` and follow its interview
methodology to determine the project's visual direction.

After the user confirms a direction, fire bootstrap with
`DESIGN_DIRECTION=[confirmed direction]`.
